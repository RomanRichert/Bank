...
import static com.telran.bank.exception.messages.ErrorMessage.AMOUNT_IS_0;
import static com.telran.bank.exception.messages.ErrorMessage.TRANSFER_AMOUNT_IS_NEGATIVE;
...
public class RequestChecker {
    ...
    public static void checkTransactionPossibility(String fromId, String toId, Double amount, Account fromAccount, Account toAccount) {
        ...
        if (Objects.equals(fromAccount, toAccount) && amount < 0 && abs(amount) > fromAccount.getAmountOfMoney().doubleValue())
        ...
        if (!Objects.equals(fromAccount, toAccount)) {
            ...
            if (abs(amount) > fromAccount.getAmountOfMoney().doubleValue())
                throw new NotEnoughMoneyException(fromId);
        }
    }
}